...
Ivry CLI returns the following exit codes:
...
Ivry CLI respects the following environment variables:

- `IVRY_API_URL`: Override the default API URL
- `IVRY_LOG_LEVEL`: Set the log level (DEBUG, INFO, WARNING, ERROR)
- `IVRY_NO_COLOR`: Disable colored output when set to any value
- `HTTP_PROXY`/`HTTPS_PROXY`: Proxy settings for network connections